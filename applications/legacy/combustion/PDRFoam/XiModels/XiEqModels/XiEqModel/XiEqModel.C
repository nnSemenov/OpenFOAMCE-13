...
:
    XiEqModelCoeffs_
    (
        XiEqProperties.typeDict
        (
            XiEqProperties.lookup<word>("XiEqModel")
        )
    ),
    thermo_(thermo),
...
{
    XiEqModelCoeffs_ = XiEqProperties.optionalTypeDict(type());
...
Foam::XiEqModel::calculateSchelkinEffect(const scalar uPrimeCoef) const
{
    ...
    const scalarField cellWidth(pow(mesh.V().primitiveField(), static_cast<scalar>(1.0/3.0)));
    ...
    (
       ...
       *sqrt
        (
            (U.primitiveField() & CT.primitiveField() & U.primitiveField())
           *cellWidth
        )
    );
